...
pub async fn whatsapp_bridge_incoming(
    ...
    tokio::spawn(async move {
        ...
        if let Some(report_topic) = extract_full_report_topic(&resolved_text) {
            ...
            services::send_whatsapp_message(&phone, &format!("Preparing research report for \"{report_topic}\"...\n\n_This typically takes 3–5 minutes. I'll update you as I go._")).await;
            ...
            match services::run_research_agent(
                ...
                Ok(report) => {
                    ...
                    if let Some(pdf_bytes) = pdf {
                        ...
                        let slug: String = topic.chars().take(40).map(|c| if c.is_alphanumeric() || c == ' ' { c } else { '_' }).collect::<String>().trim().replace(' ', "_");
                        let filename = format!("Professor_AI_{slug}.pdf");
                        services::send_whatsapp_document(&phone, "", &b64, &filename, &format!("*{topic}*\n━━━━━━━━━━━━━━━━━━━━━━\nResearch report with APA 7th citations.\n_Generated by Professor AI • v2.0_")).await;
                    } else {
            ...
            services::send_whatsapp_message(&phone, &format!("Researching \"{topic}\"...")).await;
...
I investigate topics using live data from academic sources, YouTube, web publications, and public discussion forums, then compile structured research outputs.
